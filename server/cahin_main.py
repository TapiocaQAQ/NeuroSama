...
def convert_messages_to_prompt(messages, response_type="one_sentanse"):
    """
    ...
    """
    formatted_prompt = '''<|begin_of_text|><|start_header_id|>system<|end_header_id|> Cutting Knowledge Date: December 2023'''
    message_prompt = ""
    # print(messages)
    # Process user and assistant messages
    end_prompt = "<|eot_id|>"
    role = f"<|start_header_id|>{messages[0]['role']}<|end_header_id|>"
    ...
    lang = "What's the programming language you want to learn? "
    ...
    if response_type == "one_sentanse":      # for response
        if messages[0]['role'] == "user":
            return return_message_list[0] + assistant_start_prompt
        if messages[0]['role'] == "assistant":
            return return_message_list[0]
    elif response_type == "multi_sentanse":  # constants, for response
        result_string = "".join(return_message_list)
        return formatted_prompt + result_string
...
if __name__ == "__main__":
    ...
    while True:
        ...
        def check_new_history(rag_prompt, related_history_content):
            related_history_list = []
            related_history_list = eval(f"[{related_history_content}]")
            for related_history in related_history_list:
        ...
        new_conversation.append(prompt.split("<|start_header_id|>assistant<|end_header_id|>")[0])
